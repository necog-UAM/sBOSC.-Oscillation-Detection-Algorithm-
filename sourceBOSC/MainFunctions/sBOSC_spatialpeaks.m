function  [spatialpks, localpks] = sBOSC_spatialpeaks(powspctm, thshld, cfg)
...
for trl = 1:nTrials
    ...
    for vidx = 1:length(valid_vox)
        v = valid_vox(vidx);
        pow_v = squeeze(pow_trl(v, :, :)); % [nFreqs x nTp]

        % Local peaks
        [~, ~, locmx] = findlocalmax(pow_v, 1, []);

        % Aperiodic threshold
        apthreshold = repmat(thshld(v, :)', 1, nTp);
        mask_apthreshold  = pow_v >= apthreshold;

        % Keep peaks that are local maxima and exceed the aperiodic threshold
        localpks(trl, v, :, :) = locmx & mask_apthreshold;
    end
    ...
    % Frequency loop
    for f0 = 1:nFrex
        anypeak = squeeze(localpks(trl,:, f0, :));
